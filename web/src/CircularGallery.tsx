...
class App {
  ...
  constructor(
    ...
    this.onResize();
    this.createGeometry();
    this.createMedias(items, bend, textColor, borderRadius, font, wobble);
    this.update();
  ...
  createMedias(
    ...
    this.medias.forEach(m => {
      m.program.uniforms.uWobble.value = wobble;
    });
  }
  ...
  onTouchMove(e: MouseEvent | TouchEvent) {
    ...
    this.lastClientX = x;
    if (Math.abs(x - this.start) > 8) this.moved = true;
  ...
  onTouchUp(e?: MouseEvent | TouchEvent) {
    this.isDown = false;
    this.onCheck();
    if (!this.moved && this.downOnCanvas && this.onItemClick) {
      let cx = this.lastClientX;
      if (e && 'changedTouches' in e && e.changedTouches[0]) cx = e.changedTouches[0].clientX;
      else if (e && 'clientX' in e) cx = e.clientX;
      const idx = this.hitTest(cx);
      if (idx >= 0) this.onItemClick(idx);
    }
  }

  // map a click x to the real item index (medias are duplicated, so mod itemCount)
  hitTest(clientX: number): number {
    ...
    for (const m of this.medias) {
      ...
      if (d <= half && d < bestD) { bestD = d; best = m.index; }
    }
    if (best < 0) return -1;
    return ((best % this.itemCount) + this.itemCount) % this.itemCount;
  }
...
export default function CircularGallery({
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, wobble]);
  return (
    <div
      className="cg-root"
      ref={containerRef}
